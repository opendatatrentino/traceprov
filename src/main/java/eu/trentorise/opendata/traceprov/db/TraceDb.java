...
public class TraceDb {
    ...
    public static final String ODRDB_FILE = "odrdb.json";

    private static final String FILE_PREFIX = "file://";

    public static final long ODR_ORIGIN_ID = 0L;

    private static Logger logger = LoggerFactory.getLogger(TraceDb.class);
     ...
     *
     * @param folderpath The folder where the db is located
     * @param objectMapper The object mapper to use for serialization and
     * deserialization. Normally it can be safely shared with other instances
     * except when they reconfigure it - in this case other threads must not use
     * the object mapper during reconfiguration.
    ...
    public static TraceDb connectToDb(String folderpath, ObjectMapper objectMapper) {
        logger.info("Connecting to OdrDb at " + folderpath + "   ...");
        ...
        if (json.exists()) {
        	...
        	TraceDb ret = objectMapper.readValue(json, TraceDb.class);
                ...
                logger.info("Connected to OdrDb at " + folderpath);
                return ret;
    ...
    public void drop() {
        ...
        logger.info("Dropping OdrDb... at " + dbUrl);
        ...
        if (!existsDb(dir.getAbsolutePath())) {
            throw new IllegalStateException("Tried to drop a directory which doesn't look like an OdrDb folder! " + dir.getAbsolutePath());
        }
        ...
        logger.info("Dropped OdrDb at " + dbUrl);
    }
    ...
    public void flush() {
        ...
        File outputFile = new File(dbUrl, ODRDB_FILE);
        try {
            ...
            try {
                ...
                logger.error("Error while closing odrdb file", ex);
            }

        }

        logger.info("Done flushing Odr db.");

    }

    /**
     * Returns true if there already an OdrDb in provided folder.
     */
    public static boolean existsDb(String folderPath) {
        return new File(folderPath + File.pathSeparator + ODRDB_FILE).exists();
    }
     ...
     *
     * @param folderpath The folder where the db is located
     * @throws IllegalStateException if folderpath is non empty
     */
    public static TraceDb createDb(String folderpath, ObjectMapper objectMapper) {

        logger.info("Creating OdrDb at " + folderpath + "  ...");
        ...
        if (dir.exists()) {
            if ((dir.isFile())) {
                throw new IllegalStateException("Error while creating an OdrDb, target path should be a directory, found a file instead! " + folderpath);
            }

            if (dir.list().length > 0) {
                throw new IllegalStateException("Error while creating an OdrDb, target directory is not empty! " + folderpath);
            }
        }
        ...
        logger.info("Created OdrDb at " + folderpath);
     ...
     *
     * @param prefix Notice prefix is supposed to include the colon, like
     * 'foaf:'
     * @param url i.e. http://www.w3.org/1999/02/22-rdf-syntax-ns#
     */
     ...
     *
     * @throws NotFoundException if prefix doesn't have associated any expansion
     * in the db
     */
    ...
    /**
     * Returns the object with given id. If id is not present, null is returned.
     *
     * @param viewId the odr internal id of the view to retrieve.
     */
    @Nullable
    public synchronized <C extends Object> C read(Class<C> clazz, long viewId) {
        checkNotNull(clazz);
        checkArgument(viewId >= 0);
        return (C) storedValuesById.get(clazz.getName(), viewId);
    }
    ...
    private <C extends View> C readOdrView(Class<C> clazz, long viewId) {
        ...
        if (!clique.isEmpty()) {
            ...
            if (odrView != null
                    && ODR_ORIGIN_ID == odrView.getOriginId()) {
                return odrView;
            }
        }
        return null;        
     ...
     * the provided view.
     *
     * @param originId the id of the origin server of the view to retrieve
     * @param externalId the id in the external server of the object to retrieve.
     */
    @Nullable
     ...
     * {@link #ODR_ORIGIN_ID}, then all others are tried sequentially. If url is
     * not in any context, null is returned.
     *
    ...
    public synchronized <C extends View> C read(Class<C> clazz, String url) {
        ...
        C candidate1 = read(clazz, ODR_ORIGIN_ID, normalizedUrl);
    ...
    /**
     * Returns the view with given url at given origin id. If not present, null
     * is returned.
     *
     * @param originUrl The url of the context that originated the context.
     * @param url the url of the object to retrieve.
     * @return the object with given url or null if not found.
     */
    @Nullable
    public synchronized <C extends View> C read(Class<C> clazz, long originId, String url) {
        ...
        return (C) storedValuesByUrl.get(clazz.getName()).get(originId, normalizedUrl);
     ...
     * identifier slot
     * @throws NotFoundException, or null if no mapping existed
     */
    @Nullable
    public synchronized View update(View view) {
        ...
        if (existingView == null) {
            throw new NotFoundException("Tried to update non-existing view with id " + view.getId());
        }
        ...
        checkArgument(normalizedUrl.equals(view.getUrl()),
                ...
                view.getUrl(),
                normalizedUrl);
        checkArgument(readOriginUrls(view.getOriginId()).size() > 0, "There is no stored origin with id %s", view.getOriginId());
    ...
    /**
     * States all provided view ids are 'same as'. If they are already same as with
     * some other id outside the provided ones, the old same as relation is
     * enlarged to include the new relations.
     *
     * @param clazz
     * @param ids the sameas ids
     * @return the old list of same as
     * @throws IllegalStateException if there are no corresponding views for the
     * provided ids
     */
    ...
    public void registerClass(Class clazz) {
        checkNotNull(clazz);
        checkState(isRegistered(clazz), "Class %s is already registered in %s !", clazz.getName(), TraceDb.class.getSimpleName());
    ...
    /**
     * @throws IllegalStateException if {@code clazz} is not registered
     * @param clazz
     */
     ...
     *
     * @param viewId the id of a view, which is not necessarily a view generated
     * by odr.
     * @param originId the id of the server of origin
     */
    public ControllerStatus controllerStatus(Class clazz, long viewId, long originId) {
        // The view is NEW if it is not in any equivalence relation with objects from the domain.
        throw new UnsupportedOperationException("TODO IMPLEMENT ME");
    }

    /**
     * Creates a new origin with the given url aliases (first url is the
     * prefereed one).
     *
     * @return the id of the inserted origin.
     * @throws IllegalStateException in case any of the urls is already
     * associated to an origin.
     */
    public synchronized long createOrigin(Iterable<String> urls) {
        ...
        storedOrigins = ImmutableListMultimap.<Long, String>builder()
                .putAll(storedOrigins)
                .putAll(ret, urls)
                .build();
     ...
     *
     * @throws NotFoundException if objects are not found.
     */
    public boolean shallowEqual(Class clazz, long viewId1, long viewId2) {
        ...
        if (view1 == null) {
            throw new NotFoundException("Attempted equality comparison but couldn't find first view!");
        }
        if (view2 == null) {
            throw new NotFoundException("Attempted equality comparison but couldn't find second view!");
        }
        throw new UnsupportedOperationException("TODO IMPLEMENT ME!");
    ...
    public boolean sameAs(Class clazz, long viewId1, long viewId2) {
        if (read(clazz, viewId1) == null) {
            throw new NotFoundException("Couldn't find first view id " + viewId1);
        }

        if (read(clazz, viewId2) == null) {
            throw new NotFoundException("Couldn't find second view id " + viewId2);
        }
    ...
    public boolean isSynchronized(Class clazz, long originId, String externalId) {
        ...
        if (odrView == null) {
            throw new NotFoundException("Attempted equality comparison but couldn't find main view for origin id " + originId + " and external id " + externalId);
        }
        if (foreignView == null) {
            throw new NotFoundException("Attempted equality comparison but couldn't find view for origin id " + originId + " and external id " + externalId);
        }
